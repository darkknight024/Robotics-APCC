#!/usr/bin/env python3
"""
test_reachability_singularity.py — Reachability + Singularity Analysis Test

Tests if robot end-effector can reach all waypoints in toolpaths
across robot/knife/toolpath combinations, AND performs detailed
singularity classification (shoulder / elbow / wrist) for every
reachable waypoint.

Singularity CSV reports are ALWAYS saved.  Singularity graphs (PNG)
are only generated when ``export_singularity_graphs: true`` in the
config or ``--export-singularity-graphs`` is passed on the CLI.
    ...
    └── <robot_name>/
        └── <knife_name>/
            └── <toolpath_name>/
                ...
                ├── ...
                ├── T1_singularity_report.csv        (ALWAYS)
                ├── T1_singularity_dashboard.png     (optional)
                └── reachability_rate_per_trajectory.png
    └── reachability_analysis.txt

Usage:
    python tests/test_reachability_singularity.py
    python tests/test_reachability_singularity.py --config tests/configs/reachability_config.yaml
    python tests/test_reachability_singularity.py --export-singularity-graphs
"""
...
class TrajectoryResult:
    ...
    singularity_reports: List = field(default_factory=list)  # List[SingularityReport]
...
def check_trajectory_reachability(
    ...
    Returns:
        ...
        and singularity reports for reachable waypoints
    """
    ...
    # Singularity analysis for reachable waypoints
    singularity_reports = []
...
def generate_report(
    ...
    lines.append("")
    lines.append(sep_heavy)
    lines.append("End of Reachability & Singularity Analysis Report")
    lines.append(sep_heavy)
...
def main():
    ...
    parser.add_argument('--export-singularity-graphs', action='store_true',
                        help="Generate singularity analysis plots (PNG). CSV reports are always saved.")
