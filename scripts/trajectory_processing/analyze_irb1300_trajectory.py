...
def compute_manipulability_index(jacobian, characteristic_length_m=1.0):
    """
    Compute normalized manipulability index (Yoshikawa measure).
    
    Normalize the Jacobian to make manipulability dimensionless by scaling
    ...
    Args:
        ...
        characteristic_length_m: Characteristic robot length in meters (workspace reach)

    Returns:
        manipulability: Normalized manipulability index (dimensionless)
    """
    # Normalize Jacobian: linear rows (last 3) divided by characteristic length
    # Angular rows (first 3) kept as-is (already unitless in rad)
    J_normalized = jacobian.copy()
    J_normalized[3:6, :] = J_normalized[3:6, :] / characteristic_length_m
    
    # Yoshikawa's manipulability measure on normalized Jacobian
    # Result is dimensionless (unitless)
...
def analyze_trajectory_kinematics(
    ...
    Args:
        ...
        characteristic_length_m: Workspace reach in meters for Jacobian normalization
    ...
    pbar = tqdm(total=len(trajectory_m), desc=f"  Waypoints", unit="wp", leave=False)
    ...
    for i, waypoint_m in enumerate(trajectory_m):
        ...
        if success:
            ...
            # Compute singularity measures (manipulability is normalized by workspace reach)
            manipulability = compute_manipulability_index(J, characteristic_length_m=characteristic_length_m)
...
def analyze_trajectory(urdf_path, trajectory_m, output_dir, config=None, trajectory_id=1, 
                      robot_reach_m=1.0):
    """
    Direct interface for analyzing a single trajectory.
    
    Args:
        urdf_path: Path to robot URDF file
        trajectory_m: Trajectory numpy array (n_waypoints, 7) in robot base frame (meters)
        output_dir: Output directory for results
        config: Configuration dict with analysis parameters (optional)
        trajectory_id: Trajectory identifier for logging
        robot_reach_m: Workspace reach in meters (for normalized manipulability)
    ...
    # Extract IK parameters from config and ensure proper types
    ik_config = config.get('analysis', {}).get('ik', {})
    max_iterations = int(ik_config.get('max_iterations', IK_MAX_ITERATIONS))
    tolerance = float(ik_config.get('tolerance', IK_TOLERANCE))
    ...
    # Analyze trajectory
    print(f"  Analyzing trajectory {trajectory_id}...")
    results = analyze_trajectory_kinematics(
        model, data, trajectory_m,
        trajectory_id=trajectory_id,
        q_prev_rad=None,
        max_iterations=max_iterations,
        tolerance=tolerance,
    ...
    # Print summary
    print_trajectory_summary(results, trajectory_id)
    
    # Perform joint continuity analysis
    reachable_results = [r for r in results if r['reachable']]
    ...
    if reachable_results:
        ...
        orientation_issues = analyze_orientation_constraints(results)
    ...
    # Save results
    os.makedirs(output_dir, exist_ok=True)
    save_results(results, str(output_dir), RESULTS_CSV, joint_discontinuities)
    
    # Generate plots if enabled in config
    if config.get('analysis', {}).get('visualize_plots', True):
        generate_all_analysis_plots(results, model, str(output_dir))
...
def main():
    """
    Main function to run the trajectory analysis (CLI mode).
    """
    print("=" * 70)
