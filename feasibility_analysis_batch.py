...
Runs :func:`feasibility_analysis.run_batch_single_job` for every
robot × knife × toolpath combination defined in the batch config
(same collision / IK path as the single-toolpath CLI).
...
Usage::
    ...
    python feasibility_analysis_batch.py --cspace-forbidden-yaml config/cspace_forbidden_zones_irb1300_714.yaml --cspace-only
"""
...
from feasibility_analysis import (
    CollisionRunOverrides,
    run_batch_single_job,
    _add_collision_cli_arguments,
    _collision_overrides_from_args,
)
...
def process_batch(
    ...
    coll_on = config.collision.enabled and not ov.no_collision
    if ov.cspace_only and ov.cspace_forbidden_yaml:
        print(f"Collision: C-space only ({ov.cspace_forbidden_yaml})")
    elif coll_on:
        print(f"Collision: ON (scene={config.collision.scene_yaml})")
    else:
        print("Collision: OFF")
    ...
    if num_workers <= 1:
        for i, kwargs in enumerate(task_args):
            ...
            result = run_batch_single_job(**kwargs)
        ...
        with ProcessPoolExecutor(max_workers=num_workers) as executor:
            future_to_name = {
                executor.submit(run_batch_single_job, **kwargs): Path(kwargs["toolpath_path"]).stem
...
def main():
    ...
    process_batch(
        ...
        args.output,
        args.workers,
        enable_c1=False if args.no_c1 else None,
