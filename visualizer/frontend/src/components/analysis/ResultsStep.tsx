...
export function ResultsStep() {
  ...
  const successRate =
    runResult.kind === 'feasibility'
      ? `${runResult.num_trajectories} trajectory(ies) analyzed`
  ...
  return (
    <div className="p-3 space-y-2 text-xs">
      ...
      <p className="text-xxs text-text-muted">
        {runResult.kind === 'feasibility'
          ? 'Feasibility plots are in the right panel. Use the trajectory selector if multiple paths exist.'
          : 'Kinematics plots are in the right panel. Scrub the timeline below the 3D view.'}
      </p>
