...
class RobotConfig:
    ...
    blend_shape_k: float = 0.0          # cubic-Bézier shape (0 ⇒ pipeline default)
    ...
    max_orientation_speed_deg_s: float = 0.0  # scalar TCP orientation-speed ceiling (0 ⇒ disabled)
...
def load_robots_config(config_path: str = None) -> Dict[str, RobotConfig]:
    ...
    for robot_data in config.get('robots', []):
        ...
        result[name] = RobotConfig(
            ...
            blend_shape_k=float(cal.get('blend_shape_k', 0.0)),
            ...
            max_orientation_speed_deg_s=float(cal.get('max_orientation_speed_deg_s', 0.0)),
